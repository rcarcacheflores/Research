...
xlab(c("Taxes in billions(USD)"))+ labs(title="Relationship between taxes and real GDP in the US (1984-2014)", caption = "Source: FRED")
#the syntax is similar so we just call our second plot and add the theme
p2+theme_economist()
```

![scatter plot 5]({{site.baseurl}}/assets/img/post3_p5.png)
...
(hopefully) using Python. Try to redo the thumbnail plot above using theme_calc()!
