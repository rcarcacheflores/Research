...
variables. Therefore, what we have are averages for 112 countries during
this short time period (one was removed for being a known outlier). The variables 
are coded as follow: 
...
person. 
2. Obesity prevalence seems to have a positive relationship with the average daily calorie 
intake and possibly with a country’s GDP per capita.

Now it’s time to evaluate these theories using a logit model!
...
is just one comparison criteria, but for now let's stick to the first
...
predicting countries with obesity prevalence above the global mean **within
our dataset.**
...
From the confusion matrix we need to focus on the diagonal. We correctly predicted 
about 93% of the cases. Of course, with more data we could generate 
a training and testing subset. For now, suppose we wanted to focus on predicting 
above average obesity levels for the non-member and OECD member countries, 
one of the variables which was statistically significant:
